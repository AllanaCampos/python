...
def main():
    ...
    print(f"custo antes do reajuste = {sum(salarios.Lista)}")
    novosSalarios = list( map( (lambda s : s + s*0.1), salarios.Lista ) )
    print(f"custo depois do reajuste = {sum(novosSalarios)}")
    def poeNoDia1(data):
        data.dia = 1
        return data

    datasAlteradas = list( map( poeNoDia1, filter((lambda data : data.ano < 2019), datas.Lista) ) )


    for dataAlterada, data in zip(datasAlteradas, filter((lambda data : data.ano < 2019), datas.Lista)):
        print("Data:", data," Data alterada: ", dataAlterada)
